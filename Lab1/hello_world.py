...
# Y = alpha + beta[0] * X1 + beta[1] * X2 + beta[2] * X3 + np.random.randn(size) * sigma
Y = alpha + (np.pi*np.std(X3)) * np.exp(-0.5*((X3 - np.mean(X3))/np.std(X3))**2) + np.random.rand(size) * sigma
